...
from app import db
from models import Company, Product, Inventory, Warehouse, Sale, Supplier, SupplierProduct
...
def low_stock_alerts(company_id):
    ...
    for inv, prod, wh in q:
        if prod.low_stock_threshold is not None:
            ...
            threshold = DEFAULT_THRESHOLD_BY_TYPE.get(prod.product_type_id, FALLBACK_THRESHOLD)
        else:
        ...
        recent_sales_count = db.session.query(func.count(Sale.id)).filter(
        ...
        total_sold = db.session.query(func.coalesce(func.sum(Sale.quantity), 0)).filter(
            Sale.product_id == prod.id,
            Sale.company_id == company_id,
        ...
        sup_row = db.session.query(Supplier, SupplierProduct).join(SupplierProduct, Supplier.id == SupplierProduct.supplier_id)\
            .filter(SupplierProduct.product_id == prod.id, Supplier.company_id == company_id)\
            .order_by(SupplierProduct.lead_time_days.asc()).first()
